                ...
                <table class="table table-striped">
                    ...
                    <tbody v-else-if="entities.length > 0">
                        <tr v-for="(commande, index) in entities" v-bind:key="commande.id">
                            ...
                            <td>{{ commande.cl.nom }}</td>
                            <td>{{ commande.adresse_livraison ?? "Non définie" }}</td>
                            <td><Status :value="commande.status" /></td>

                            <td class="d-flex justify-content-center">
                                <router-link v-if="true" :to="{ name: 'commande.client.voir', params: { id: commande.id }}" class="btn btn-info btn-sm me-2 text-white"><i class="fa fa-eye"></i></router-link>
                                <router-link v-if="true" :to="{ name: 'commande.client.modifier', params: { id: commande.id }}" class="btn btn-primary btn-sm me-2"><i class="fa fa-edit"></i></router-link>
...
export default {
    ...
    setup() {
        ...
        const confirmDeletion = (id, index) => {
            SimpleAlert.confirm("Voulez-vous supprimer ce commande ?", "Question", "question").then(() => {
        ...
        onMounted(() => {
            getEntities({ type: 2, appro: false }) // Recuperer tous les commandes de clients
        })
