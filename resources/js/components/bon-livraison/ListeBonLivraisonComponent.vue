    ...
    <table class="table table-striped">
        ...
        <tbody v-if="entities.length > 0">
            <tr v-for="(livraison, index) in entities" :key="livraison.id">
                ...
                <td class="align-middle">{{ livraison.date }}</td>
                ...
                <td class="align-middle">
                    <Status :value="parseInt(livraison.status)" />
                </td>

                <td class="d-flex justify-content-center">
                    <router-link title="Voir ce bon de livraison" v-if="true"
                        :to="{ name: `bon-livraison.voir`, params: { id: livraison.id } }"
                    ...
                    <DeleteBtn title="Supprimer ce bon de livraison" v-if="true" type="danger"
                        @click.prevent="confirmDeletion(livraison.id, index)" />
                </td>
            </tr>
        </tbody>
        <tbody v-else>
...
export default {
    ...
    setup(props: { entities: any[] }) {
        ...
        const confirmDeletion = async (id: number, index: number): Promise<any> => {
            await SimpleAlert.confirm("Voulez-vous supprimer le bon de livraison ?", "Suppression", "question").then(() => {
